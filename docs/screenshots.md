...
Full panel with **MCP Bridge** connected (green status), selection, and feature modules.
...
Gear menu: model settings, prompt settings, update check, and **中文 / English** language toggle.
...
OpenAI-compatible **API base URL**, model name, API key, and **Test connection**.
...
**Export** tab: 1× preview, editable filename, single download, and **ZIP** pack (3× PNG).
...
Editable system prompts for rename / group (`{{candidates}}` and related placeholders).
...
Compact window: bridge status + current selection only (use the minimize control in the header).
...
After a successful connect, Cursor shows **figma-agent-mcp** with **37 tools enabled**.
...
- [Exporting slices](./exporting-slices.md)
